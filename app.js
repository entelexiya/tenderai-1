...
(() => {
  ...
  function render(r) {
    ...
    side.append(el("h3", "", "Экспериментальная модель"));
    side.append(
      el(
        "p",
        "muted",
        r.ml.status === "experimental"
          ? `Аномальность: ${r.ml.anomaly_index} / 100. ${r.ml.note}`
          : r.ml.note ||
              "ML отключён. Выполнена только проверка текстовыми правилами.",
      ),
    );
